...
// Htmx Component
export const HtmxComponent = defineComponent({
  name: 'Htmx',
  props: {
    tag: {
      type: String,
    ...
    // Catch-all for HTMX attributes
    ...Object.fromEntries(
      ['get', 'post', 'put', 'patch', 'delete', 'trigger', 'target', 'swap', 'select', 'sync',
        'include', 'exclude', 'params', 'pushUrl', 'replaceUrl', 'indicator', 'confirm',
        'prompt', 'timeout', 'disable', 'sync', 'history', 'historyElt', 'inherit', 'disinherit']
        .map(prop => [`hx${prop.charAt(0).toUpperCase()}${prop.slice(1)}`, { type: [String, Object, Boolean] }])
    ),
    // Raw HTML content
    html: String
  },
  ...
  setup(props, { slots }) {
    ...
    watch(
      ...
      (newProps, oldProps) => {
        ...
        Object.keys(oldProps).forEach(key => {
          if (key === 'tag' || key === 'html') return

          const attrKey = key.replace(/([A-Z])/g, '-$1').toLowerCase()
          const finalAttrKey = attrKey.startsWith('hx-') ? attrKey : `hx-${attrKey}`
          element.value.removeAttribute(finalAttrKey)
        })
        ...
        Object.keys(newProps).forEach(key => {
          ...
          if (value === true) {
            element.value.setAttribute(finalAttrKey, '')
          } else {
            element.value.setAttribute(finalAttrKey, value)
          }
        })

        // Re-process element with HTMX
        if (window.htmx) {
    ...
    onUnmounted(() => {
      if (element.value && window.htmx) {
        window.htmx._('cleanupElement')(element.value)
      }
    })

    return () => h(props.tag, { ref: element }, slots.default?.() || (props.html ? { __html: props.html } : null))
  }
})
